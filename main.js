...
class Countries {
  ...
  toSocContacts(){
    this.Canada.forEach(human=> {
      ...
      if( human.status == 'healthy' ){
        ...
        if(human.socialContacts < 0){human.socialContacts = 0}
      }
  ...
  ifKnowsThatInfected(){
    this.Canada.forEach(human=>{
      if( human.status == 'healthy' ){
        ...
        if(human.responsibility < 0){human.responsibility = 0}
      }
      else if( human.status == 'sick!Knows'){
        ...
        human.responsibility = human.responsibility - Math.exp(randomInteger(0,5))
        if(human.responsibility < 0){human.responsibility = 0}
      }
      else if( human.status == 'inTreatment'){
        human.responsibility = human.responsibility - Math.exp(randomInteger(0,5))
  ...
  efficiencyOfHealing(){
    this.Canada.forEach( human => {
        if( human.status == 'inTreatment' ){
          human.efficiencyOfHealing = randomInteger(0,15) + Math.floor(randomInteger(0,1.5))
          if(human.efficiencyOfHealing > 100){ human.efficiencyOfHealing = 100}
        }
    ...
    this.France.forEach( human => {
      if( human.status == 'inTreatment'){
        human.efficiencyOfHealing = randomInteger(0,9) + Math.floor(randomInteger(0,1.5))
        if(human.efficiencyOfHealing > 100){ human.efficiencyOfHealing = 100}
      }
      else if(human.status == 'healthy'){
        human.efficiencyOfHealing = 0
    ...
    this.Germany.forEach( human => {
      if( human.status == 'inTreatment'){
        human.efficiencyOfHealing = randomInteger(0,12) + Math.floor(randomInteger(0,1.5))
        if(human.efficiencyOfHealing > 100){ human.efficiencyOfHealing = 100}
      }
      else if(human.status == 'healthy'){
        human.efficiencyOfHealing = 0
    ...
    this.USA.forEach( human => {
      if( human.status == 'inTreatment'){
        human.efficiencyOfHealing = randomInteger(0,16) + Math.floor(randomInteger(0,1.5))
        if(human.efficiencyOfHealing > 100){ human.efficiencyOfHealing = 100}
      }
      else if(human.status == 'healthy'){
        human.efficiencyOfHealing = 0
  ...
  contagiousness(){
    this.Canada.forEach( human => {
      if( human.status == 'inTreatment' ){
        ...
        if(human.contagion < 0){ human.contagion = 0 }
      }
      else if( human.status == 'sick!Knows'){
      ...
      else{human.contagion = 0}
    })
  }
  toRecover(){
      this.Canada.forEach( human => {
        if( human.status == 'healthy' ){
          human.recoveryTime = 1 + randomInteger(20 - 1.5*human.health - 0.5*human.efficiencyOfHealing,5-0.25*human.health-0.1*human.efficiencyOfHealing) 
          if(human.recoveryTime < 0){human.recoveryTime = human.efficiencyOfHealing * 1.5 +' days'}
        }
        else if( human.status == 'sick!Knows'){
          human.recoveryTime = 2 + randomInteger(30 - 2.2*human.health - 0.8*human.efficiencyOfHealing,4-0.15*human.health-0.12*human.efficiencyOfHealing) 
          if(human.recoveryTime < 0){human.recoveryTime = human.efficiencyOfHealing * 1.5 +' days'}
        }
        else if( human.status == 'sickKnows'){
          human.recoveryTime = 3 + randomInteger(10 - 0.4*human.health - 0.3*human.efficiencyOfHealing,4-0.2*human.health-0.15*human.efficiencyOfHealing) 
          if(human.recoveryTime < 0){human.recoveryTime = human.efficiencyOfHealing * 1.5 +' days'}
        }
        else if( human.status == 'inTreatment'){
          human.recoveryTime = 4 + randomInteger(25 - 2*human.health - 0.2*human.efficiencyOfHealing,2-0.1*human.health-0.1*human.efficiencyOfHealing) 
          if(human.recoveryTime < 0){human.recoveryTime = human.efficiencyOfHealing * 1.5 +' days'}
        }
      ...
      this.Canada.forEach( human => {
        if(Number.parseInt(human.recoveryTime) < 0){ human.recoveryTime = Number.parseInt(human.recoveryTime) * 2 }
      })
  }
  ifDead(){
    this.Canada.forEach( human => {
      if( human.status == 'healthy'){
        human.mortality = human.age * 0.05 + ((15-1.5*human.health - 0.5*human.efficiencyOfHealing)+(3-0.25*human.health-0.1*human.efficiencyOfHealing))
        if( human.mortality < 0){ human.mortality = -human.mortality / 5 }
      }
      else if(human.status == 'sick!Knows'){
        human.mortality = human.age * 0.07 + ((15-1*human.health - 0.8*human.efficiencyOfHealing)+(2-0.15*human.health-0.12*human.efficiencyOfHealing))
        if( human.mortality < 0){human.mortality = -human.mortality /5}
      }
      else if(human.status == 'sickKnows'){
        human.mortality = human.age * 0.09 + ((5-0.35*human.health - 0.3*human.efficiencyOfHealing)+(2-0.3*human.health-0.15*human.efficiencyOfHealing))
        if( human.mortality < 0){human.mortality = -human.mortality /5}
      }
      else if(human.status == 'inTreatment'){
        human.mortality = human.age * 0.11 + ((5-0.05*human.health - 0.2*human.efficiencyOfHealing)+(2-0.1*human.health))
        if( human.mortality < 0){human.mortality = -human.mortality /5}
      }
    })
  }
  infectedCount(){
    ...
    let count = 0;
    this.Canada.forEach( human =>{
      if( human.status == 'sickKnows' || human.status =='sick!Knows' || human.status == 'inTreatment')
      {
        countCan++
      }
    })
  ...
  startLiving(){
    this.Canada.forEach( human => {
      if(human.status =='sickKnows' || human.status == 'sickKnows' || human.status == 'inTreatment'){
        let friends = [];
        let friend;
...
for( let i = 1; i <infected.length; i++){
  if(infected[i] > 0){
    ...
    console.log(`Математическое отклонение за ${i} день: ${expect - infected[i]}`)
  }
  if(infected[i] <= 0){
    console.log(`Количество людей, которые вылечились за ${i} день: ${-infected[i]}`)
    let expect = infected[i] + error;
    console.log(`Математическое ожидание вылечившихся людей за ${i} день: ${ -expect}`)
    console.log(`Математическое отклонение вылечившихся за ${i} день: ${expect - (-infected[i])}`)
